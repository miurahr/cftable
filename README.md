...
また、PNG/JPEG等の画像形式のグラフ出力もサポートしています。
...
2.  **ローカルファイル名の使用**: 個人用ファイルには `*.local.yaml` または `*.private.yaml` という名前を付けることを推奨します。これらはデフォルトで `.gitignore` の対象となっており、誤ってコミットされるのを防げます。
    ```bash
    cp template/example_input.template.yaml my_plan.local.yaml
    ```
...
4. **LibreOfficeで分析**: ODS形式の出力ファイルは、LibreOffice Calcや、Google Spreadsheet, Microsoft Excel, JustSystem 花子 等 で開くことができます。資産推移や年齢別の資産残高を視覚的に確認するだけでなく、内訳の数字をできます。
...
# PNG形式のグラフ出力
cftable my_plan.local.yaml -o output.private.png
```
